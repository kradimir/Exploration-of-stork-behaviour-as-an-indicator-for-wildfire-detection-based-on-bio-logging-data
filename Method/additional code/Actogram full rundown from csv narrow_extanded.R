...
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Brigitte_A9V53__eobs_8019_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Bruno_9705_E0661.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Emanuel_A9V72__eobs_7344_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Franzl___DER_AZ026__eobs_7204_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Leon_HL452__3064_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Loriana____A3M19__e_obs_7990_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Mira_____DER_AU650__eobs_3027_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Mose_XA122__3994_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Odette___A3M04__eobs_7347_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Olli___DER_A1P74__e_obs_6382_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Silas_____DER_A7N66__e_obs_7045_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Teamy___AT771__eobs_6381_.csv"
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_2/Overlap_Ursula_A9V57__eobs_8048_.csv"
...
file_path <- "D:/Vladimir/Narrowed down overlap analysis on few events/overlap analysis result_3/Overlap_Odette___A3M04__eobs_7347_.csv"
...
for (cur_animal in individuals) {
  ...
  for (i in 1:nrow(interesting_events)) {
    ...
    start_time <- event_time - days(5)
    end_time <- event_time + days(5)
    ...
    plot_filename <- paste0("D:/Vladimir/Narrowed down overlap analysis on few events/interesting_event_TRUE_csv_using_narrow_data_wit_extanded_time_around_event/actogram", safe_animal_name, "_", format(event_time, "%Y-%m-%d_%H-%M-%S"), ".jpg")
    
    # Open the JPG device with higher resolution
    ...
    segments(y0 = as.numeric(format(cur_time_start, format='%H')) + as.numeric(format(cur_time_start, format='%M')) / 60, y1 = as.numeric(format(cur_time_end, format='%H')) + as.numeric(format(cur_time_end, format='%M')) / 60, x0 = cur_time_start, x1 = cur_time_end, pch = '2', lwd = 11, col = Speed_Colors(length(Breaks) - 1)[findInterval(cur_speeds, Breaks)])
    ...
    # Plot black dots when within_1km is TRUE
    points(as.numeric(format(Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == TRUE], format='%H')) + as.numeric(format(Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == TRUE], format='%M')) / 60 ~ Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == TRUE], pch = 21, col = 'black', cex = fire_point_size[Dataset_bird_event$Within_1km == TRUE], lwd = 0.5)
    
    points(
      as.numeric(format(Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == TRUE], format='%H')) + 
    ...
    # Plot black dashes when within_1km is FALSE with a constant size
    points(as.numeric(format(Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == FALSE], format='%H')) + as.numeric(format(Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == FALSE], format='%M')) / 60 ~ Dataset_bird_event$timestamp[Dataset_bird_event$Within_1km == FALSE], pch = '-', col = 'black', cex = 0.8)

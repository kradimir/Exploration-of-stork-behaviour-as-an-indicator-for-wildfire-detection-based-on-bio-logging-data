#pipeline to make actrograms of other birsd in the same region as Leon (Chad/African center republic)
#no fire only actograms
...
#this regions is around south east chad, where we want to see actograms
#lat_min <- 5.804013
#lat_max <- 14.272900
#long_min <- 14.775717
#long_max <- 25.189642

#sub Saharan Africa above Uganda without the coasts
# Top left corner
lat_max <- 16.478230
long_min <- -12.362012
# Bottom right corner
...
# Load the data (assuming combined_stork_data is already loaded)
# If not, you would load it using:
load("D:/Vladimir/Vladimir/FF_STK_dataset_fusion_ONLY_combined_stork_Data_post_2020_environment.RData")

# Filter the data for the specified geographical zone
selected_region_data <- combined_stork_data %>%
  filter(
    location.lat >= lat_min & location.lat <= lat_max,
    location.long >= long_min & location.long <= long_max
  )

# Ensure selected_region_data is an sf object before dropping geometry
selected_region_data <- st_drop_geometry(selected_region_data)

# View the new dataframe
str(selected_region_data)
print(unique(selected_region_data$individual.local.identifier))
summary(selected_region_data)

# Make data usable by actogram plot
...
# Add a date column extracted from the timestamp
data <- data %>%
  mutate(date = as.Date(timestamp))

# Define the target month and day
target_month_day <- "01-08"

# Initialize the interesting_event column to FALSE
data$interesting_event <- FALSE

# Process each bird's data separately
data <- data %>%
  group_by(individual.local.identifier) %>%
  arrange(date) %>% # Ensure data is sorted by date
  group_modify(~ {
    .x$interesting_event <- FALSE  # Ensure interesting_event is re-initialized within each group
    # Get unique years in the data for the current bird
    unique_years <- unique(year(.x$date))
    for (yr in unique_years) {
      # Find the first occurrence of January 8th in the current year
      target_date <- as.Date(paste(yr, target_month_day, sep = "-"))
      idx <- which(.x$date == target_date)[1]
      if (!is.na(idx)) {
        .x$interesting_event[idx] <- TRUE
      }
    }
    return(.x)
  }) %>%
...
# Filter out individuals with no interesting_event = TRUE
data <- data %>%
  group_by(individual.local.identifier) %>%
  filter(any(interesting_event == TRUE)) %>%
  ungroup()

sum(data$interesting_event, na.rm = TRUE)
#if bigger than 0 it worked
#here = 5 meaning there's 5 instance of birds (every individual local identifier being a 8th of January in the region)
...
All_locs_no_gaps_good_birddates <- All_locs_no_gaps_df %>%
  ...
  filter(p_less_than_5 > 0.95 | interesting_event == TRUE) %>%
...
for (cur_animal in individuals) {
  ...
  Dataset_bird_cur <- All_locs_no_gaps_good_birddates %>%
    ...
    select(location.long, location.lat, timestamp, interesting_event) %>%
    drop_na(timestamp)
  ...
  for (i in 1:nrow(interesting_events)) {
    ...
    plot_filename <- paste0("D:/Vladimir/Actograms/", interval, " days_", safe_animal_name, "_", format(event_time, "%Y-%m-%d_%H-%M-%S"), ".jpg")    
    # Open the JPG device with higher resolution
    jpeg(filename = plot_filename, width = 3200, height = 2400, res = 200)
    ...
    points(as.numeric(format(interesting_events$timestamp, format='%H')) + as.numeric(format(interesting_events$timestamp, format='%M')) / 60 ~ interesting_events$timestamp, pch = 4, col = 'red', cex = 0.5, lwd = 1)

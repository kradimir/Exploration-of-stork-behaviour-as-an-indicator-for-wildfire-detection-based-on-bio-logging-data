...
for (cur_animal in individuals) {
  ...
  for (i in 1:nrow(interesting_events)) {
    ...
    plot_filename <- paste0("D:/Vladimir/Narrowed down overlap analysis on few events/interesting_event_TRUE_csv_using_narrow_data_wit_extanded_time_around_event/", interval, " days_", safe_animal_name, "_", format(event_time, "%Y-%m-%d_%H-%M-%S"), ".jpg")    
    ...
    segments(y0 = as.numeric(format(cur_time_start, format='%H')) + as.numeric(format(cur_time_start, format='%M')) / 60, y1 = as.numeric(format(cur_time_end, format='%H')) + as.numeric(format(cur_time_end, format='%M')) / 60, x0 = cur_time_start, x1 = cur_time_end, 
             pch = '2', lwd = 11, col = Speed_Colors(length(Breaks) - 1)[findInterval(cur_speeds, Breaks)])
